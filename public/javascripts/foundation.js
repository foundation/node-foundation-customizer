...
if ($('[data-delicious-links]').length > 0) {
  ...
  $.ajax({
    url:'http://feeds.delicious.com/v2/json/zurb/foundation?count=15',
    dataType:'jsonp',
    success: cb
  });
}
...
if ($('[data-foundation-blog]').length > 0) {
  ...
  $.ajax({
    url:'http://zurb.com/blog/rss/foundation/json?count=10&topic=foundation',
    dataType:'jsonp',
    crossDomain: 'true',
    success: cb
  });
...
//
// var repositionStickyNav = function(scrollTop) {
//   var stickySidebar = $('.sticky'),
//       maxScrollWindow = $('.main-content').height() - $('.main-content').offset().top,
//       topOffset  = $('.main-content').offset().top;
//
...
//       });
//     }
//     else if (scrollTop > topOffset && scrollTop < maxScrollWindow) {
//       stickySidebar.css({
//         'position': 'fixed',
//         'top': '0'
//       });
...
//       stickySidebar.css({
//         'position': 'absolute',
//         'top': maxScrollWindow - topOffset + 'px'
//       });
//     }
//   }
// }
...
if ($('[data-building-blocks]').length > 0) {
  ...
  $.ajax({
    url:'http://zurb.com/library/api/building_blocks/type/all?count=12',
    dataType:'jsonp',
    success: cb
  });
}
...
if ($('[data-lessons]').length > 0) {
  ...
  $.ajax({
    url:'http://zurb.com/university/api/v1/lessons?count=3',
    dataType:'jsonp',
    success: cb
  });
}
...
dragDropArea.on('dragenter', function(e) {
  e.stopPropagation();
  e.preventDefault();
  $(this).css('border', '2px solid #0B8CBA');
});

dragDropArea.on('dragleave', function(e) {
  e.stopPropagation();
  e.preventDefault();
  $(this).css('border', '2px dashed #0B8CBA');
});
...
// Handle uploading of Hosted demo
function uploadDemoResume(file) {
...
$('#skateForm').on('submit', function(e){
  ...
  $.post("http://zurb.com/ink/skate-proxy.php", data, function(resp){
  ...
  if (email) {
    ...
    $.post("http://zurb.com/ink/newsletter.php", data, function(resp){$('#emailBox').html('<div data-alert class="alert-box">Congratulations, '+email+' is signed up for the list!<a href="#" class="close">&times;</a></div>');});
  }
});
...
function app_stargazers_check(){
  if ($('#stars').hasClass('apps')){
    ...
    return "https://api.github.com/repos/zurb/foundation?callback=foundationGithub&access_token=8e0dfc559d22265208b2924266c8b15b60fd9b85";
  }
}
$.ajax({
  ...
  success: function (response) {
    if (response && response.data.watchers) {
      ...
      $('#stars').html(watchers + 'k stargazers');
