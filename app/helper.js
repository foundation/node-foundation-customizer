...
exports.createCompleteAndEssential=function(){
  ...
  .then(function(){
    ...
    var fork = childProcess.spawn(process.env.SHELL, ['-ci',essential]);
    ...
    fork.on('close', function(code){
      var forker = childProcess.spawn(process.env.SHELL, ['-ci', 'gulp sass:foundation && gulp javascript:foundation && gulp deploy:custom'], {cwd: 'assets/temp-essential'});
        ...
        var archive2 = archiver('zip');
          outputer.on('close', function () {
            ...
            var go = function(){
              ...
              var forkeroo=childProcess.spawn(process.env.SHELL,['-ci', 'rsync -av --progress ../../foundation-sites-6/* ../../f6'])
              ...
              forkeroo.on('close',function(){
                var forkier=childProcess.spawn(process.env.SHELL,['-ci', 'git reset --hard'], {cwd: '../../f6'})
